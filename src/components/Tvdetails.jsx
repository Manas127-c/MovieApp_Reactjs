...
import { infosasync, removtv } from "../store/reducer/tvSlice";
    ...
    >
      ...
      <div className="w-full min-h-[100vh] overflow-y-auto lg:mt-0 mt-5">
        <div className="w-[100vw] h-[70vh] relative">
          ...
          <div className=" absolute lg:w-[50vw] w-[90vw] lg:h-[60vh] h-[50vh] lg:left-[33%] left-[5%] lg:top-[5%] top-[50%] flex flex-col justify-center lg:items-start items-center py-5 lg:gap-6 gap-3">
            <h1 className="lg:text-4xl text-2xl font-bold text-white">
              ...
              <span className="font-medium lg:text-2xl text-lg text-zinc-400 ml-2">
            ...
            <div className="flex lg:gap-2 gap-1 items-center justify-start text-white">
              ...
              <h1 className="text-white font-normal lg:text-lg text-xs">
                {info.details.first_air_date || "Not Availiable"}{" "}
            ...
            {info.videos ? (
              <Link
                ...
                className=" text-md px-4 py-2 w-32 flex items-center justify-center bg-[#6556CD] rounded text-white mt-4 font-semibold hover:bg-violet-500 duration-300"
              >
                Watch Trailer
              </Link>
            ) : (
              <h1 className="text-white text-xl font-semibold">
                Trailer Not Availiable
              </h1>
            )}
          </div>
        </div>
        <div className="lg:w-[80vw] w-[95vw] min-h-[70vh] overflow-y-auto mx-auto lg:mt-5 mt-32">
          <h1 className="ml-4 mb-4 text-zinc-400 text-xl font-semibold">
            Seasons
          </h1>
          <hr className="my-3 " />
          {info.details.seasons.map((s, i) => (
            <div
              key={i}
              className="lg:w-[70%] w-[100%] lg:h-[30vh] h-[25vh] mt-5 rounded flex item-start justify-center gap-6 p-3 hover:bg-zinc-950 duration-300"
            >
              <img
                className=" w-[35%] h-[95%] object-cover object-top rounded"
                src={
                  s.poster_path
              ...
              <div className="w-[60%] h-full flex flex-col items-start justify-start">
                <h1 className="lg:text-2xl text-lg font-bold text-yellow-600">
                  {s.name}
                  <span className="font-medium lg:text-xl text-sm text-zinc-400 ml-2">
                    ({s.air_date})
                  </span>
                </h1>
                <h1 className="my-2 text-white lg:text-md text-sm font-regular">
                  Total episodes :{" "}
                  <span className="text-blue-500">{s.episode_count}</span>
                </h1>
                <h1 className="lg:text-xl text-sm text-white font-semibold">Overview</h1>
                <p className="mt-1 text-white lg:text-md text-xs font-normal line-clamp-3">
                  {s.overview.length > 0
                    ? s.overview
                    : "Overview not availiable"}
                </p>
              </div>
            </div>
          ))}
        ...
        <div className="lg:w-[80vw] w-[100vw] h-[40vh] lg:mt-5 mt-2  mx-auto lg:p-5 p-1">
          <h1 className="ml-4 text-zinc-400 text-xl font-semibold">
            More Like This
          </h1>
          <hr className="my-3" />
          <Horizontalcards data={info.similar} tit="tv" />
        </div>
        <div className="lg:w-[80vw] w-[100vw] h-[40vh] lg:mt-5 mt-2  mx-auto lg:p-5 p-1">
          <h1 className="ml-4  text-zinc-400 text-xl font-semibold">
            More Recomendations
          ...
          <hr className="my-3" />
          {info.recomendations.length > 0 ? (
            <Horizontalcards data={info.recomendations} tit="tv" />
          ) : (
